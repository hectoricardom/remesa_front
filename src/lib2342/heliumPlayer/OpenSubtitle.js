...
import {  CloseModal , SrtCachingFile } from '../../actions/common'
import './style.css';
...
const readFile = (event,fileState,setFileState,setObs) => { 
...
const fileCheckInput = (e,fileState,setFileState,setObs) => { 
 let  _fileState = fileState || {};
 const {label} =_fileState;
 var input = e.target;
 if(input && input.files && input.files[0]){
  ...
  reader.addEventListener('load',(e)=> readFile(e,fileState,setFileState,setObs));
  reader.readAsText(file);
 }
}


const labelInputFocus = (e,fileState,setFileState,setObs) => { 
  let  _fileState = fileState || {};
  _fileState['focus'] = true;
  setFileState(_fileState);  
}

const labelInput = (e,fileState,setFileState,setObs) => { 
  let  _fileState = fileState || {};
...
const labelInput_Blur = (e,fileState,setFileState,setObs) => { 
  let  _fileState = fileState || {};
  const {url,fileBlob} =_fileState;  
  let file = true;
  _fileState['label'] = e.target.value;
...
const OpenSubtitle = (props) => { 
    ...
    const [obs,setObs] = useState(_Util.gen12CodeId());   
    ...
    return(
      <>
      <div  {...data.modalID?{"dialog-key-id":data.modalID}:""}  className={`something--wrong--Dialog _${modalID}`} style={{minHeight:`295px`}}>
        <div>              
        ...
        <div>
          ...
          <input type="text" 
            onChange={(e)=>labelInput(e,fileState,setFileState,setObs)} 
            onFocus={(e)=>labelInputFocus(e,fileState,setFileState,setObs)}
            onBlur={(e)=>labelInput_Blur(e,fileState,setFileState,setObs)}
            {..._value} 
          />
        </div>
        {_file?
        <div className={`upload--Wrapper`}>
          <div className={`file_Done`}>{fileBlob?<Icon2 name={'done'} color={'#fff'} size={'1.6em'}/>:null}</div>
          <div className="upload--Container"  onDrop={(e)=>dropHandler(e)} onDragOver={(e)=>dragOverHandler(e) }>          
            ...
            <input type={`file`} style={inputFileStyle} id={_Id} onChange={(e)=>fileCheckInput(e,fileState,setFileState,setObs)} accept=".vtt,.srt"/>              
          </div>
        </div>: 
        <div>
