...
const useObserve = () => {
  ...
  return {  updKV, _dispatch_ ,observe,  close, updForms,_Login }
}
...
const LOGIN = (props) => { 
  ...
  const {data} = props;  
  const { observe,  close,  updKV  } = useObserve();
  ...
  const Logout = () => { 
    window.localStorage.setItem('hrm_auth_token_media',null);
  ...
  return(    
    <div  {...data.modalID?{"dialog-key-id":data.modalID}:""} className={`_${modalID}  something--wrong--Dialog `} >
      <div>              
          <h3 className="title"><span>{item["username"]}</span></h3>
      ...
      <div className="loginWh_Container">        
        <div className={`loginWh_--message-fieldset  visible`}>
          <h4 className="loginWh_--details-header">
            <span className="loginWh_--details-question">{item["isActive"]?"su cuenta esta activa":"su cuenta esta desactivada"}</span>
            <span className="loginWh_--details-question-optional">{''}</span>
          </h4>         
        </div>
        <div className={`_dsplFlx `}>
          <div className="flexSpace"/>  
          <button  type="submit" aria-label="Enviar informe" className="loginWh_--submit-button" onClick={(i)=>Logout()}><span>Cerrar Sesion</span></button>
        </div>
