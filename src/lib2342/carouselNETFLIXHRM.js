  ...
  const {      
   ...
   if(props.loop){
      ...
      if(animating){
         if(action===1){
            // let diff = total-_index;
            // console.log('diff2ck',diff,tabs,diff-tabs)
            //  if(diff<tabs){            let diffWidth = diff*itemWidth;     }
            if(_index-(tabs)>=0){ 
               _3XTranslate=  XTranslate+100;
            }else{           
               _3XTranslate= XTranslate+100;
            }
         ...
         if(action===1){
            if(_index<tabs){
               let diff = _index-tabs;              
               _3XTranslate= tabs+diff===0?null: XTranslate-((diff/tabs)*100);
            }else{
               _3XTranslate=  XTranslate;
            }
         }else if(action===2){          
            _3XTranslate=  _index<tabs ? -100 : XTranslate;
         }
      }

      //console.log("translate3d", _3XTranslate)
               ...
               <div className={`sliderContent ${animating && 'animating'}`} style={carousel_element_Style}>
                  {
                     range2Render2.map((in3,ii3)=>{   
                        ...
                        let _idS = `${in3}_sld_${y[in3]}`;
                        return (
                           <div className={` slider-item ${isInRange?y[in3]:""}`} 
                              id={_idS}
