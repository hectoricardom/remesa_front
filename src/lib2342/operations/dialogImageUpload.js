...
import {  CloseModal, ImageFileUpload  } from '../../actions/common'
...
const useObserve = () => {
  ...
  return { observe,  close,  dispatch, getState }
}
...
const UploadImage = (props) => {  
  const {data} = props;  
  const { observe,  close,  dispatch, getState } = useObserve();
  ...
  let imagesUpload = _state["imagesUpload"];
  
  let pUpl = _state["uploadProgressImage"] ||  0;
  let _ID = item["id"];



  const fileCheckInput = (e) => {
    var formName = "_uploadImage"; 
    var UploadUrl = `/UploadImage?videoRef=${_ID}`;
    ...
    filess.map(fl=>{
      ImageFileUpload(UploadUrl, [], fl, formName, dispatch, _state, _ID );
    })
    close(modalID)
  } 
  ...
  return (
    <>
     <div style={{"--progress_upload":pUpl+'%'}} className={` A9Uzve g3VIld V639qd J9Nfi`} {...data.modalID?{"dialog-key-id":data.modalID}:""}> 
        ...
        <span className={` x1LMef `} > {`Upload Image`} </span>  
          <div className={`fieldPadding`}>      
            <div  className={` _upload_video_  `}  onDrop={(e)=>dropHandler(e)} onDragOver={(e)=>dragOverHandler(e)}>    
              <input type={`file`} style={{opacity:`0`,height:`100%`,width:`100%`}} multiple={true} id={uploadID} onChange={(e)=>fileCheckInput(e, _ID, dispatch, getState)}  
              accept="image/*" 
