...
import { addEpisode2Season } from '../../actions/videos'
...
const formName = "newVideo";



const typeList = [
  {id:"movie",name:"movie"},
  {id:"episode",name:"episodio"},
...
const handleValidate = (e, setValid, forms, field ) => {
  ...
  let fld2Prs = ['videoRef','number'] ;
  let addValidate = {
    videoRef: {required:true,minLength:3,maxLength:50},
    number:{required:true, number:true, minValue:1, maxValue:300}
  }
...
const handleSave = (_addVideo, forms, modalID,seasonParent ) => {  
  let fld2Prs = ['videoRef','number'] ;
  let addValidate = {
    videoRef: {required:true,minLength:3,maxLength:50},
    number:{required:true, number:true, minValue:1, maxValue:300}
  ...
  return (
    ...
    <div className={` A9Uzve g3VIld V639qd J9Nfi`}  {...data.modalID?{"dialog-key-id":data.modalID}:""}> 
      ...
      <div className={` _frmWrp `}>
        <div style={{position:'relative',marginTop: `10px`}} onClick={()=>{}}  className={`fieldPadding`}>
          <InputText 
            ...
            validations={{required:true, number:true, minValue:1, maxValue:300}} 
        ...
        <div className={`_VideoDetail `}>
          <InputText
            ...
            OnChange={(e)=>handleValidate(e, setValid, forms,  `videoRef` ) } 
          />
        </div>
      </div>
      ...
      <div className={` XfpsVe `}>
        ...
        {valid && seasonParent && 
        <span className={`_separateBtn`} onClick={()=>handleSave(_addVideo, forms, data.modalID, seasonParent)}>
