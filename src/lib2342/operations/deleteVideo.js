...
import { RmvVideo } from '../../actions/videos'
...
const useObserve = () => {
  ...
  const _rmVideo = (id,modalID, _ID) => {
    let qry = forms && forms["videoSearch"] && forms["videoSearch"]["search"]
    RmvVideo({id:id},dispatch,"videoSearch", qry?qry:"")
  ...
  return { observe,  close, _rmVideo }
}

const RmvComponent = (props) => {  
  const {data} = props;  
  const { observe,  close,  _rmVideo } = useObserve();
  const item = data.item || {};
  let modalID = data.modalID; 
  let _ID = data.dID;  
  // let Rdxuse = useObserve();
  let _name = item && item['originalFile'] && item['originalFile']['name']
                      
  return (
    <>
      <div className={` A9Uzve g3VIld V639qd J9Nfi`}  {...data.modalID?{"dialog-key-id":data.modalID}:""}> 
        <div className={` oEOLpc tOrNgd R6Lfte `}>
          <span className={` x1LMef `} > {` ¿Desea eliminar el video `}<span  className={`_textLbl_`}>{_name}</span>{`, una vez eliminado no podra ser recuperado?`} </span>        
        </div>     
        ...
        <div className={` XfpsVe `}>
          ...
          <span className={`_separateBtn`} onClick={()=>_rmVideo(item.id, modalID, _ID)}>
            <BTN_f  theme={`fire_brick`} title={'Delete'}/>
          </span>
         
        </div>
      </div>      
      <style>{AddfileMedia_styles}</style>
    </>
  )
}
...
export default RmvComponent




const AddfileMedia_styles = `
