...
import { RmvVideoFile } from '../../actions/videos'
...
const useObserve = () => {
  ...
  const _rmvFile = (id,modalID, _ID) => {
    let qry = forms && forms["fileVideoSearch"] && forms["fileVideoSearch"]["search_file"]
    RmvVideoFile({id:id},dispatch,"fileVideoSearch", qry?qry:"")
  ...
  return { observe,  close, _rmvFile }
}

const RmvfileMediaComponent = (props) => {  
  const {data} = props;  
  const { observe,  close,  _rmvFile } = useObserve();
    ...
    <>
      <div className={` A9Uzve g3VIld V639qd J9Nfi`}  {...data.modalID?{"dialog-key-id":data.modalID}:""}> 
        <div className={` oEOLpc tOrNgd R6Lfte `}>
          <span className={` x1LMef `} > {` ¿Desea eliminar el archivo  `}<span  className={`_textLbl_`}>{_name}</span>{`, una vez eliminado no podra ser recuperado?`} </span>        
        </div>     
        ...
        <div className={` XfpsVe `}>
          ...
          <span className={`_separateBtn`} onClick={()=>_rmvFile(item.id, modalID, _ID)}>
...
export default RmvfileMediaComponent
