...
const handleValidate = (e, setValid, forms, field ) => {
  ...
  let fld2Prs = ['title','type'] ;
  let addValidate = {
    title: {required:true,minLength:3,maxLength:50},
    type: {required:true,minLength:3,maxLength:50}
  }
  ...
  return (
    ...
    <div className={` A9Uzve g3VIld V639qd J9Nfi`}  {...data.modalID?{"dialog-key-id":data.modalID}:""}> 
      <div className={` oEOLpc tOrNgd R6Lfte `}>
        <span className={` x1LMef `} > {`Crear Nuevo Video `} </span>
      </div>  
      <div className={` _frmWrp `}>
        <div style={{position:'relative',marginTop: `10px`}} onClick={()=>{}}  className={`fieldPadding`}>
          <DropBtn 
            ...
            keyCode={10}
            field={"type"}
            OnSelect={(e)=>handleValidate(e, setValid, forms,  "type" ) }
          />
        </div>
        <div className={`_VideoDetail `}>
          <InputText
            ...
            OnChange={(e)=>handleValidate(e, setValid, forms,  `title` ) } 
          />
        </div>
      </div>
