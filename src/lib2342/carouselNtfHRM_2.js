...
const CarouselHRM = (props) => {
  ...
  const {      
   ...
   if(props.loop){
      ...
      let _start = _index-(tabs+1);
      
      let _end  = _index+(tabs*2)+(_index>0?2:1);
      if(action===1){  
         
         let diff = slidesItems-_index;
         console.log(slidesItems,diff)
      ...
      if(action===2){
         ...
         _start = _index>tabs?_index-tabs:0;         
         console.log("action",diff,tabs,_start )
         _end = _end>total?total:_end;         
         if(diff<tabs){
            _start = total-(tabs*2);
         }
      ...
      if(animating){
         if(action===1){           
            ...
            //console.log("XTranslate",index,XTranslate ,dd,animatingValue)
            let XTranslateAnimating = dd?dsdds: _index<tabs ? animatingValue*(_index>0?2:1) : XTranslate +  100;
            _3XTranslate= XTranslateAnimating;
            //console.log("Next_3XTranslate",index,_3XTranslate )
         }         
      }else{
         if(action===1){           
            ...
            //let diff = slidesItems-(_index+tabs);
            //let dd = diff>tabs?0:diff;
            let XTranslateAnimating = _index<tabs ? -100 : XTranslate;
            _3XTranslate= XTranslateAnimating;
         }
      }
         ...
         if(!animating){
            ...
            setTimeout(()=>{
               ...
               if(props.loop){
                  ...
                  if((slidesItems-index)<tabs){
                     console.log("prev",NewIndex,slidesItems-index,(slidesItems-index)+tabs);
                     setIndex((slidesItems-index)+tabs); 
                  }
                  else if(NewIndex>=0){
                     setIndex(NewIndex); 
                  }
...
*/
   let _prevBtn = props.loop ? true: _index>0 ? true:false;
   let _nextBtn = props.loop ? true: _index+tabs<slidesItems-1 ? true:false;
   // console.log("***************************")
   let inStr = null;
   ...
   return(
      <>    
         ...
         >
            {_prevBtn && 
            <span className="handle handlePrev active" tabindex="0" role="button" aria-label="Ver títulos anteriores"  onClick={()=>handlePrev()}>
            ...
            {_nextBtn && 
            <span className="handle handleNext active" tabindex="0" role="button" aria-label="Ver más títulos" onClick={()=>handleNext()}>
