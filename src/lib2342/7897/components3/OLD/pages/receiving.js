...
const operationType = 'receiving'

const formName = "_receiving_";
...
const useObserveChanges = () => {
  ...
  const products2receiving =  useSelector(state => state.products2receiving); 
...
const AddProduct2Receive= (validAdd, e, setProductList, forms, _getProductsByCode, setValidForm ) => {
  if(validAdd){
    ...
    _getProductsByCode(prodID, "products2receiving")
    // console.log('AddProduct2Receive',prodID)
 ...
 */
  ...
  if(deptID){
    ...
    // console.log(products2receiving)
    ...
    let _list = departments[deptID]['list'].sort(function(a, b) {
  ...
  let locationsList = {};
  let productListValid = ValidateProductList(productList,products2receiving);
          ...
          <div className={`_dsplFlx _wrp_Flx` }>
              <div className={`fieldPadding`}>
                <InputText 
                  ...
                  OnChange={(e)=>handleValidForm( e, setValidForm, forms, formName,"document" )}  
                />
              </div>
              <div className="flexSpace"/>  
              <div className={`fieldPadding`}>
                <InputText 
                  ...
                  OnChange={(e)=>handleValidForm( e, setValidForm, forms, formName, "trackingNumber" )}  
                />
              </div>
        ...
        <div className={'_dsplFlx _desktopV '} >
          ...
          <div className={`fieldPadding`}>
            <InputText 
              ...
              index={2} 
              keyCode={21} 
              validations={{required:true, number:true, minValue:0.01, maxValue:50000}} 
              placeholder={_Util.translatetext(77)}
              OnChange={(e)=>handleValidAdd(e,setValidAdd,forms,`${formName}_prod`,"price")}  
              //initValue={ssFomrm?ssFomrm['product']:null}
            />
          </div>
          <div className="flexSpace"/>  
          <div className={`fieldPadding`}>
            <InputText 
              ...
              index={2}  
              keyCode={22}
              validations={{required:true, number:true, minValue:0.01, maxValue:5000}} 
              placeholder={_Util.translatetext(316)}
              OnChange={(e)=>handleValidAdd(e,setValidAdd,forms,`${formName}_prod`,"qty")}  
              //initValue={ssFomrm?ssFomrm['product']:null}
            />
          </div> 
              ...
              <div className={'payment_details  _total2 _sectionsItem'}>             
               <span>Items</span>
                   ...
                   productList && _Util.ObjectKeys(productList).map(slcId=>{
                     ...
                     let isValidP = dasdg0smpdhg(products2receiving, slc['productID']);
                      ...
                      if(slc){
                        ...
                        return(
                          ...
                          <div className={'payment_details  _stock _qty_'}>
                            <div  className={'__amount__ __gm2_text_ qty'}>  {`${slc['qty']} X `}</div> 
                            <div  className={'__amount__ __gm2_text_'}>  ${slc['price'] && slc['price'].toFixed(2)}</div> 
                          </div>
...
export default withRedux(ReceivingComponent);
