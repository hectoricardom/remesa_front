...
const DepartmentViewDetail = (props) => {
  const { 
  ...
  let _stock = totalStock(_sections);
    ...
    return (
      ...
      <Layout />
        ...
        <div  className={'_dsplFlx _mrg15 _locationsDepartment_ _desktopV'}>  
          ...
          <div className={'_actions_Location_'}>
            <div className={'_actions_Btns_'}>
              <div className={`IconRippleEffectContainer edit`}  onClick={()=>OpenEditDepartment(item, _ID)}>
                <Icons name={'outline_edit'} color={'#555'} size={24}/>
              </div>
              <div className={'_spaceMrg_'}/>
            {!_stock  && <div className={`IconRippleEffectContainer delete`} onClick={()=>OpenDeleteModalHRM(item)}>
        ...
        {item && item.id && 
        <div className={`_dsplFlx _departmentDetails_`}>
          <MenuSlideSide options={options} updFiltersTab={(e)=>setView(e)} />
          <div className={"locationWrp"}>
            ...
            <div  className={'_dsplFlx _mrg15 _sections _locations_dept_'}>                   
              <div className={'payment_details  _total2 _sectionsItem'}>             
                { sc && sc.name==="inventory" && <span onClick={()=>OpenAddLocation(sc.id, `${_name}-${sc.name}`, _ID)}>
                    <BTN_HRM theme={`light_blue`} title={'Add Location'}/>
                  </span>    
                } 
                    {
                    sc.locations.map(slc=>{
                      let _stock = slc['stockDetails'] && slc['stockDetails']['inStock']?slc['stockDetails']['inStock']:0;
                      
                      return(
                        <div  className={'_dsplFlx _mrg15 _location_'} key={_Util.gen12CodeId()}>                    
                          <div className={'payment_details  _total2 '}>                            
                          ...
                          <div className={'_actions_Location_'}>
                            <div className={'_actions_Btns_'}>
                              ...
                              <div className={'_spaceMrg_'}/>
                              <div className={``}  onClick={()=>OpenEditLocation(sc.id, `${_name}-${sc.name}`, _ID,slc)}>
                                  <Icons 
                                    name={'outline_edit'} 
                                    color={'#555'} 
                                    size={24}
                              ...
                              <div className={'_spaceMrg_'}/>
                              <div className={``} onClick={()=>OpenDeleteLocation(slc, _ID)}>
                                <Icons 
                                    name={'outline_delete'} 
                                    color={'#555'} 
                                    size={24} 
...
export default withRedux(DepartmentViewDetail)
