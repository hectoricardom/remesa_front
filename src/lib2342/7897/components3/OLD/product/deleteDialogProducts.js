...
import { RmvProducts } from '../../actions/products'
...
const useObserve = () => {
  ...
  const _rmvProducts = (id,modalID) => {
    RmvProducts({id:id},dispatch,_state,operationType)
  ...
  return { observe,  close, _rmvProducts }
}

const RmvProductsComponent = (props) => {  
  const {data} = props;  
  const { observe,  close,  _rmvProducts } = useObserve();
    ...
    <>
      <div className={` A9Uzve g3VIld V639qd J9Nfi`}  {...data.modalID?{"dialog-key-id":data.modalID}:""}> 
        ...
        <div className={` XfpsVe `}>
          ...
          <span className={`_separateBtn`} onClick={()=>_rmvProducts(item.id, modalID)}>
...
export default RmvProductsComponent
