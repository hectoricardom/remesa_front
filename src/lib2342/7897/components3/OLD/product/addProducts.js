...
import { AddProducts, UpdProducts } from '../../actions/products'
...
const useObserve = () => {
  ...
  const _updProducts = (doc,operation,dID) => {     
    UpdProducts(doc, dispatch, _state, operation,dID);
  }

  const _addProducts = (doc,operation,dID) => {
    AddProducts(doc, dispatch, _state, operation,dID);
  }


  return { observe, list, forms, clearForms, close, _updProducts, _addProducts }
}
...
const handlerSaveForm = (FuseObserve, props, router) => {
  const {  close, forms, clearForms, _addProducts, _updProducts } = FuseObserve;
  const {data} = props; 
  ...
  if(_Valid.valid){
    if(_2s.id){
      _updProducts(_2s,operationType,dID);
      clearForms(formName);
      
      //forms[formName]={};   var scroll = scrollPosition;window.scrollTo(0,scroll);
    }else{
      _addProducts(_2s,operationType,dID);
      clearForms(formName);
...
const AddProductsComponent = (props) => {  
  const {data} = props;  
  const { observe, forms, clearForms, close, _addProducts, _updProducts } = useObserve();
  ...
  return (
    < >
      ...
      <style>{AddProducts_styles}</style>
    </>
  )
}
